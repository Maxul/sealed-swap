...
#if 1// this optimization worsen the test in some way
...
static int vram_write(const char* path, const char* buf, size_t size, off_t off, fuse_file_info* fi) {
    ...
    size_t write_ret = session->file->write(off, size, buf);
    printf("%s %d, size = %ld, ret = %ld\n", __func__, __LINE__, size, write_ret);
    return write_ret;
}
...
int main(int argc, char* argv[]) {
	...
	if (mlockall(MCL_FUTURE | MCL_CURRENT)) // ensure the whole heap is locked
		perror("mlockall");
